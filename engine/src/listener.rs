...
pub async fn start_listener(
    ...
    subscription_rx: &mut UnboundedReceiver<String>  // NEW: Dynamic Subscriptions (Borrowed for loop reuse)
) {
    ...
    let (ws_stream, _) = match connect_async(&ws_url).await {
        Ok(s) => s,
        Err(e) => {
            tracing::error!("❌ WebSocket Connection Failed: {}. Retrying with backoff...", e);
            tokio::time::sleep(tokio::time::Duration::from_secs(10)).await; // Conservative backoff
            return;
        }
    };
    ...
    tracing::info!("👂 Listener ACTIVE ({} pools). Listening for dynamic adds...", monitored_pools.len());

    // 2. Process Incoming Messages AND Dynamic Subscriptions
    loop {
        tokio::select! {
            // A. Dynamic Subscription Requests
            Some(new_pool_addr) = subscription_rx.recv() => {
                tracing::info!("🆕 Dynamic Subscription Request: {}", new_pool_addr);
                let msg_id = req_id;
                req_id += 1;
                pending_subs.insert(msg_id, new_pool_addr.clone());
                ...
                if let Err(e) = write.send(Message::Text(subscribe_msg.to_string().into())).await {
                    tracing::error!("❌ Failed to send dynamic subscription for {}: {}", new_pool_addr, e);
                }
            }
